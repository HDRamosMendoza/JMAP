$(document).ready(function () {
    ...
    $('#ID_FECHA_INICIO, #ID_FECHA_FIN').datetimepicker({ locale: 'es', format: 'DD/MM/YYYY', useCurrent: false });
    ...
    /* Listar ROL */
    /*
    let _listarAuditoria = function() {
        try {
            TB_AUDITORIA.clear().draw();
            const ALERT_ERROR = `
                <div class="alert alert-danger" role="alert" style="margin-top: 20px;">
                    <strong>Error</strong> al listar la auditoría.
                </div>`;
            $.ajax({
                "url": "http://localhost:8099/ctrlAcceso/listaByDate",
                "method": "GET",
                "headers": {
                ...
                "data": JSON.stringify({
                    "fechaIni": "2022-12-19 23:53",
                    "fechaFin": "2023-01-01 01:11"
                })
            })
            ...
            console.error(`_listarAuditoria: ${error.name} - ${error.message}`);
        }
    };
    _listarAuditoria();
    */
    ...
    _elementById("ID_AUD_BUSCAR").addEventListener("click", function() {
        try {
            TB_AUDITORIA.clear().draw();
            const ALERT_ERROR = `<hr><div class="alert alert-danger" role="alert" style="margin-top: 20px;">
                    <strong>Error</strong> al listar auditoría.
                </div>`;

            const ID_SEARCH = "#ID_SEARCH_AUDITORIA";
            const ALERT_WARNING = `
                <div class="alert alert-warning" role="alert" style="margin-top: 20px;">
                    <strong>Completar</strong> datos de búsqueda.
                </div>`;
            let _fechaInicio= $("#ID_FECHA_INICIO").val();
            let _fechaFin   = $("#ID_FECHA_FIN").val();
            let _campo      = $("#ID_FC_CAMPO").val(); _campo = _campo.trim();
            let _activeTab = $('.nav-tabs a.active').text();
            //let _data = JSON.stringify({ "fechaIni": "2022-10-19 23:53", "fechaFin": "2022-12-19 23:53" });
            ...
            if(true) {
                ...
                $.ajax({
                    ...
                    "data": JSON.stringify({
                        "fechaIni": "2022-12-19 23:53:00",
                        "fechaFin": "2022-12-19 23:53:00"
                    })
                })
                .done(function(response) {
                    try {
