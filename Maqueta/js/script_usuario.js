$(document).ready(function () {
    ...
    /* let _searchCIP = function(_codigo) {
        try {
            //"url": "http://localhost:8099/auth/existeEmail",
              $.ajax({
                "url": "http://localhost:8099/auth/existeNombreUsuario",
                "method": "GET",
                "headers": {
                    "Authorization": `Bearer Bearer ${TOKEN_PNP}`,
                    "Content-Type": "application/json"
            ...
            console.error(`_searchCIP: ${error.name} - ${error.message}`);
        }
    }
    _searchCIP("231170"); */
    ...
    _elementById("ID_USU_EDI_GUARDAR").addEventListener('click', function() {
        try {
            ...
            const ALERT_ERROR = `<hr>
                <div class="alert alert-danger" role="alert" style="margin-top: 20px;">
                    <strong>ERROR</strong> al actualizar.
                </div>`;
            const ALERT_SUCCESS = `<hr>
                <div class="alert alert-success" role="alert">
                    <strong>USUARIO ACTUALIZADO</strong>. Proceso finalizado.
                </div>`;
    ...
    $('#ID_Modal_USUARIO').on('show.bs.modal', _formUsuario());
    ...
    */
    ...
    let _limpiarRol = function() { _formRol(); }
    $('#ID_Modal_ROL').on('show.bs.modal', _formRol());
    ...
    $('#ID_Modal_EDI_ROL').on('show.bs.modal', _formEdiRol());
});