$(document).ready(function () {
    ...
    $('#ID_FECHA_INICIO').datetimepicker({ locale: 'es', format: 'DD/MM/YYYY'});
    $('#ID_FECHA_FIN').datetimepicker({ locale: 'es', format: 'DD/MM/YYYY', useCurrent: true });    
    ...
    _formatDate = function(_date) {
        try {
            let dateRanger = null;
            dateRanger = new Date(_date);
            console.log("FECHA RANGO");
            console.log(dateRanger);
            let getYear = dateRanger.toLocaleString("default", {year: "numeric"});
            let getMonth = dateRanger.toLocaleString("default", {month: "2-digit"});
            let getDay = dateRanger.toLocaleString("default", {day: "2-digit"});
    ...
    _elementById("ID_AUD_BUSCAR").addEventListener("click", function() {
        try {
            ...
            let _campo      = $("#ID_FC_CAMPO").val().trim();
            _campo = _campo.trim();
            ...
            if(_activeTab == "Otros criterios") {
                ...
                /* let _inicio = _formatDate(_fechaInicio); let _fin = _formatDate(_fechaFin); */
                if ((_fechaInicio.match(RegExPattern)) && (_fechaInicio!='') && (_fechaFin.match(RegExPattern)) && (_fechaFin!='')) {
                    console.log("Rango de Fecha");
                } else {
                    $(ID_SEARCH).html(ALERT_FECHA);
                    $(ID_SEARCH).show();
                    setTimeout(function() { $(ID_SEARCH).hide(); }, 3000);
                ...
                let [_diaInicio, mesInicio, anioInicio] = _fechaInicio = _fechaInicio.split('/');
                let [_diaFin, mesFin, anioFin] = _fechaFin = _fechaFin.split('/');
                /* TABLA */
                $.ajax({
                    "url": `http://localhost:8099/ctrlAcceso/listaByDate/${anioInicio}-${mesInicio}-${_diaInicio} 00:00/${anioFin}-${mesFin}-${_diaFin} 23:59`,
                    "method": "GET",
                    "headers": {
                        "Authorization": `Bearer Bearer ${TOKEN_PNP}`,
                        "Content-Type": "application/json"
        ...
        } catch(error) { console.error(`ID_AUD_BUSCAR: ${error.name} - ${error.message}`); }
    });
