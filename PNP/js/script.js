...
const LYR = [1,2,3,7,16,22]; /* All Layer */
/* LOCAL STORAGE */
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `
                            <button type='button' class='btn btn-sm btn-general ${_cantVisitasPnp>0?"btn-warning":"btn-secondary"}' data-codigo='${_idDenuncia}' id='ID_${_idDenuncia}' onclick='_datatableVisitas(this)' title='RELACIÓN DE VISITAS PNP REALIZADAS'><i class='fa fa-user' aria-hidden='true'></i></button>
    ...
    try {
        ...
        fetch("./json/config.json")
            ...
            .then(jsondata => {
                ...
                let clave = _list.map(item => {if(item.value != "00") { return item.value}});
                ...
                idVista.addEventListener('change', function(event) {
                    ...
                    if(USUARIO_SID !== "null" && USUARIO_SID !== "" && USUARIO_SID !== null) { 
                        ...
                        if(clave.indexOf(event.target.value) != -1) {
                            JMap.Layer.setVisible(LYR_ID_DENUN,false);
                            /* Otorgamiento de Medidas de Proteccion */
                            ...
                            JMap.Layer.setVisible(LYR_ID_RIESG,false); 
                            JMap.Layer.setVisible(LYR_ID_VISIT,false);
                            JMap.Layer.setVisible(LYR_ID_DENUN,true);
                            pie.style.display = bar.style.display = "none";
                        }
                    }
                });
                /* Defecto - Selector */
                $('#ID_Vista').val('00').trigger('change');
                //pie.style.display = "block";
...
let _activarTematica = function(_boolCB) {
    try {
        ...
        idVista.addEventListener('change', function() {
            ...
            if(this.checked) {
                $('#ID_Vista').val('00').trigger('change');
                document.querySelector('.canvas-pie').style.display = 'none';
                document.querySelector('.canvas-bar').style.display = 'none';
            }
        });
    ...
    try{
        ...
        $.ajax({
            ...
            try {
                ...
                $(ID_MED_PRO).append(_rowDetalle("Cantidad de Visitas según Media de Protección",_res["cantidadVisitas"] || ""));
                $(ID_MED_PRO).append(_rowDetalle("Auto Medidas",_res["dAutoMedidas"] || ""));
                $(ID_MED_PRO).append(_rowDetalle("Estado de Vigencia",_res["dEstadoVigencia"] || ""));
                $(ID_MED_PRO).append(_rowDetalle("Otorga de Medidas",_res["dOtorgaMedidas"] || ""));
                /* Se registrar la información que se esta visualizando */
                _medidasProteccion_CTRL(_codigoDenuncia, "CONSULTA MEDIDA PROTECCION");
            } catch(error) {                
...
let _datatableVisitas = function(_this) {
    try {
        ...
        let _codigoDenuncia = _this.dataset.codigo || 0;
        _denunciaID = _this.dataset.codigo;
        $(ID_VISITAS).html("");
        ...
        /*_medidasProteccion_CTRL(_codigoDenuncia, "Consulta Visita");*/
    } catch(error) {
...
let _changeColor = function(_id,_bool) {
    
    let _nodeID = _elementById(`ID_${_id}`);
    _nodeID.disabled = !_bool;
    _nodeID.classList.remove("btn-secondary");
    _nodeID.classList.add("btn-warning");
    /*
    _elementById(`ID_${_id}`).addEventListener('click', function() {
        if(_bool) {
            this.disabled = false;
...
let _codigoDenuncia_punto;
let _visitasPunto = function(_this) {
    try {
        _elementById('Tab-3').click();
        const ID_VISITAS = "#Tab-3_TAB .tab-list";
        $("#Tab-3_TAB .tab-register").show();
        _codigoDenuncia_punto = _this.dataset.codigo || 0;
       
        $(ID_VISITAS).html("");
        /* Asignar Código de Denuncia */
        _elementById("ID_VISITA_DENUNCIA").innerText = _codigoDenuncia_punto;
        /* Visita GUARDAR */
...
_elementById("ID_VISITA_GUARDAR").addEventListener('click', function(event) {
            ...
            if(_idVisitaDenuncia == "" || _latitud == "0" || _latitud == "0" || _latitud == "" || _longitud == "" || _longitud == "" || _fecha == "0" || _documento == "" || _hechos == "" || _victimas == "" || _menores == "") {
                $(ID_VISITA).html(`<br><br><hr>
                    <div class="alert alert-warning" role="alert">
                        <strong>Advertencia</strong>. (*) Falta registrar campos obligatorios !
                    </div>`);
                $(ID_VISITA).show();
            ...
            .done(function (response) {
                try{
                    JMap.Map.refreshLayerById(LYR_ID_VICTI);
                    JMap.Layer.setVisible(LYR_ID_VICTI, true);
                    $(ID_VISITA).html(`<br><br><hr><div class="alert alert-success" role="alert">
                        <strong>REGISTRO</strong>. Grabado con éxito !
                    </div>`);
                    $(ID_VISITA).show();
                    _elementById("ID_VISITA_LIMPIAR").click();
                    _changeColor(_codigoDenuncia_punto,true);
                    _medidasProteccion_CTRL(_codigoDenuncia_punto, "CREA VISITA");
                } catch(error) {
                    $(ID_VISITA).html(`<br><br><hr><div class="alert alert-danger" role="alert">
                        <strong>ERROR</strong>. Al grabar datos !
                    </div>`);
                    setTimeout(()=>{$(ID_VISITA).hide();},5000);
    ...
    try {
        ...
        if(_indexVisita !== "") {
            ...
            .done(function (response) {
                ...
                $(ID_VISITA).hide();
                _medidasProteccion_CTRL(_elementById("ID_CODIGO_DENUNCIA").value, "BORRA VISITA");                
                setTimeout(function() {
                    let _nodeID = _elementById(`ID_${_denunciaID}`);
                    if(TB_VISITAS.rows()[0].length > 0) {
                        _nodeID.disabled = false;
                        _nodeID.classList.remove("btn-secondary");
                        _nodeID.classList.add("btn-warning");
                    } else {
                        _nodeID.disabled = true;
                        _nodeID.classList.add("btn-secondary");
                        _nodeID.classList.remove("btn-warning");
                    }
                },1000);  
...
_elementById("ID_VISITA_POSICION").addEventListener('click', function() {
    try {
        ...
        JMap.Event.Map.on.click("selectLocation", (res) => {
            if(!_boolean) {
                _elementById("ID_LATITUD").innerText = res.location.y;
                _elementById("ID_LONGITUD").innerText = res.location.x;
