$(document).ready(function () {
                    ...
                    { 
                        ...
                        render: function (data, type, row) {
                            ...
                            return `<button type='button' class='btn btn-xs btn-outline-warning my-2 my-sm-0' 
                                data-toggle='modal' data-id='${_id}' data-cip='${_cip}' data-target='#ID_Modal_EDI_USUARIO' role="tab" style="padding:0 8px;font-size:14px;">
                                <i class="fa fa-edit"></i> Editar
                            </button>
                            <button type='button' class='btn btn-xs btn-outline-danger my-2 my-sm-0' 
                                data-toggle='modal' data-id='${_id}'  data-cip='${_cip}'data-target='#ID_Modal_DEL_USUARIO' role="tab" style="padding:0 8px;font-size:14px;">
                ...
                try {
                    response.forEach((item,index) => {
                        ...
                        TB_USUARIOS.row.add([
                            ...
                            item["email"],
                            item["password"] || "********",
                            [item["idUsuario"],item["cipUsuario"]],
                        ]).draw(false);
                    });
                } catch(error) {
    ...
    _elementById("ID_USU_EDI_GUARDAR").addEventListener('click', function() {
        try {
            ...
            .always(function(jqXHR,textStatus) {
                _medidasProteccion_CTRL(_cip, "EDITA USUARIO");
    ...
    */
    ...
    $('#ID_Modal_DEL_USUARIO').on('show.bs.modal', function(_this) {
        try {
            $("#ID_USU_DEL_ID").val('');
            let _id = $(_this.relatedTarget).data().id;
            let _cip = $(_this.relatedTarget).data().cip;
            $("#ID_USU_DEL_ID").val(_id);
            $("#ID_USU_DEL_CIP").val(_cip);
        } catch(error) {
    ...
    _elementById("ID_USU_DEL_GUARDAR").addEventListener('click', function() {
        try {
            ...
            let _cipUsuario = $("#ID_USU_DEL_CIP").val();
            $.ajax({
                ...
                _medidasProteccion_CTRL(_cipUsuario, "BORRAR USUARIO");
                /*let _eval = eval(`([${response}])`);
                let _data = _eval[0].data;*/
