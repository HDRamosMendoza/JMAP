$(document).ready(function () {
    ...
    _elementById("ID_CONTENT_HIDE").style.display="none";
    ...
    let _popup = function() {
        try {
            _elementById("ID_POPUP_USUARIO").innerText = USUARIO_NOMBRE || 'Error';
            _elementById("ID_POPUP_COD_UNIDAD").innerText = USUARIO_COD || 'Error';
            _elementById("ID_POPUP_NOM_UNIDAD").innerText = USUARIO_PNP || 'Error';;
            _elementById("ID_POPUP_COD_COMISARIA").innerText = (USUARIO_SID !== "null" && USUARIO_SID !== "" && USUARIO_SID !== null) ? USUARIO_SID : 'NINGUNO';
            _elementById("ID_POPUP_NOM_COMISARIA").innerText = _validateJurisdiccion() ? "NO ASIGNADA A COMISARÍA" : USUARIO_PNP;
        } catch(error) {
            console.error(`popup: ${error.name} - ${error.message}`);
        }
    };
    _popup();    
    /* Ocultar barra de la derecha */
    let _hide = function () {
        try {
            if(_validateJurisdiccion()) {
                _elementById("ID_CONTENT_HIDE").style.display="block";
                _elementById("ID_TAB_JURISDICCION").style.display="none";
            } else {
                _elementById("ID_CONTENT_SHOW").style.display="block";
            }
        } catch(error) {
    ...
    _elementById("ID_COMISARIA").innerText = _validateJurisdiccion() ? "NO ASIGNADA" : USUARIO_PNP;
    /* Valida */
    _validateLocal();    
